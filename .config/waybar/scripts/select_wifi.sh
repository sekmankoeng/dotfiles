...
if nmcli connection show "$SSID" >/dev/null 2>&1; then
  if ! nmcli connection up "$SSID"; then
    notify-send -a "waybar" -u normal "WiFi" "连接失败：$SSID，该连接配置异常，请尝试删除后重新连接"
    exit 1
  fi
  notify-send -a "waybar" -u normal "WiFi" "已连接 $SSID"
  exit 0
fi
...
if nmcli device wifi connect "$SSID" password "$PASSWORD";then
  ...
  nmcli connection delete "$SSID" >/dev/null 2>&1 || true
  notify-send -a "waybar" -u normal "WiFi" "连接失败：$SSID，密码错误或无法接入"
  exit 1
fi
